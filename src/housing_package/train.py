...
def train_models():
    """This function train all the models like linear regression,random forest etc.
    ...
    Returns
    -------
        Creates pickle files for all the models and store those files in artifacts directory.
    ...
    output_file = os.path.join(ARTIFACTS_PATH, "lin_reg_op.pkl")
    ...
    output_file = os.path.join(ARTIFACTS_PATH, "tree_reg_op.pkl")
    with open(output_file, "wb") as f:
    ...
    output_file = os.path.join(ARTIFACTS_PATH, "new_cvres.pkl")
    with open(output_file, "wb") as f:
        pickle.dump(cvres, f)
